#!/usr/bin/env bash
# Deploy of release 0.3.0 — the contact profile (merge of
# claude/user-profile-view-e7qr9y, PR #1): client/index.html, client/style.css,
# client/app.js, client/account.js and client/contacts.js changed; the relay
# changed only its reported VERSION.
#
...
# convenient (see deploy/release-2026-09-24.md).
#
# Run from the repo root, on master:  bash deploy/deploy-2026-09-24.sh
set -euo pipefail
...
grep -q 'id="contactSheet"' client/index.html || { echo "this checkout is not the 0.3.0 client (no contact sheet)"; exit 1; }
grep -q 'VERSION = "0.3.0"' backend/config.py || { echo "backend/config.py is not 0.3.0"; exit 1; }
...
ssh "$BOX" 'curl -s http://127.0.0.1:8000/healthz | sed "s/^/    healthz: /" \
  && curl -s http://127.0.0.1:8000/healthz | grep -q "\"version\":\"0.3.0\"" \
  && curl -s -o /dev/null -w "index=%{http_code}\n" http://127.0.0.1:8000/ \
  && echo "--- the served client must be the 0.3.0 one:" \
  ...
  && curl -s http://127.0.0.1:8000/app.js | grep -c "function openContact" | sed "s/^/    app.js openContact (want 1): /" \
  && curl -s http://127.0.0.1:8000/style.css | grep -c "contact-sheet" | sed "s/^/    style.css contact-sheet lines (want >0): /" \
...
echo "        no-cache, so a reload is enough): Users -> tap a saved user -> the"
echo "        profile sheet. Then rebuild the APK when convenient."
echo
