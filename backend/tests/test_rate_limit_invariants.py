...
Every /auth/challenge request is charged to the per-client_key GLOBAL bucket
BEFORE the per-username one is consulted, so an attacker's total spend is capped
at CHALLENGE_GLOBAL_RATE_REFILL_PER_SEC however they aim it. Keeping one victim's
...
attacker's reach. At the original 0.5/s it was eight accounts; at 2.0/s it is two.
"""
import os
import sys
import tempfile
...
def _reset_limiters():
    ...
    accounts._challenge_global_limiter._buckets.clear()
    yield


def test_targeted_lockout_reach_is_bounded():
    """The item 19 invariant itself."""
    reach = config.CHALLENGE_GLOBAL_RATE_REFILL_PER_SEC / config.CHALLENGE_RATE_REFILL_PER_SEC
    assert reach <= MAX_SIMULTANEOUS_VICTIMS, (
        f"one attacker could hold {reach:.0f} accounts offline at once "
        f"(global {config.CHALLENGE_GLOBAL_RATE_REFILL_PER_SEC}/s ÷ per-username "
...
def test_global_ceiling_keeps_the_relay_usable():
    ...
    assert config.CHALLENGE_GLOBAL_RATE_REFILL_PER_SEC >= floor, (
        f"the relay-wide challenge ceiling is "
        f"{config.CHALLENGE_GLOBAL_RATE_REFILL_PER_SEC}/s, below the "
...
def test_per_username_bucket_is_not_the_global_ceiling():
    ...
    """
    assert config.CHALLENGE_RATE_REFILL_PER_SEC < config.CHALLENGE_GLOBAL_RATE_REFILL_PER_SEC, (
...
def test_pending_challenge_store_cannot_be_flooded():
    ...
    """
    worst_case_pending = config.CHALLENGE_GLOBAL_RATE_REFILL_PER_SEC * config.CHALLENGE_TTL_SEC
